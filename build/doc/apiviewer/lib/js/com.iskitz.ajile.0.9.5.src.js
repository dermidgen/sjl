...
new function/*com.iskitz.ajile*/()
{
   ...
   function $create(instance)
   {
      ...
      currentModuleName = QNAME;

      if(!isDOM)
         ((INFO.fullName  = QNAME)
         ,(INFO.path      = "../lib/")   // Default to current dir less imposing
   ...
   function AddImportListener(moduleName, listener)
   {
      ...
      else if(moduleName && (String != moduleName.constructor))
            return false;
   ...
   function cloakObject(obj)
   {
      ...
      var name     = (/(function\s*.*\s*\(.*\))/).exec(obj.toString()) || ['']
        , funkshun = name.length > 1 ? name[1] : name[0];
      ...
      {
         return (DEBUG && !CLOAK) ? funkshun : obj.prototype.toString();
      };
   }
...
paths:for(var path in paths)
   ...
   function handleImport(shortName, fullName, url, notation, version, module, owner)
   {
      ...
      if(url == null || typeof url == "undefined" || url.constructor != String)
         url = (  typeof nsInfo      != "undefined"
               &&        nsInfo      != null
      ...
      if(notation == null || typeof notation == "undefined")
   ...
   function handleImportLoaded(shortName, fullName, module, owner)
   {
      ...
      var isInclude   = shortName == fullName || pendingName == fullName;
   ...
   function hasNamingConflict(shortName, fullName, owner)
   {
      ...
      var msg = "\nWARNING: There is a naming conflict, " + shortName
              + " already exists.\nConsider using ImportAs with an alias; for "
...
//**/    alert("LOADER: "+getMainLoader(document)+"\n\nisSimple: "+isSimple+"\nisDOM: "+isDOM+"\nisDOM05: "+isDOM05+"\nisDOM1: "+isDOM1+"\nisDOM2: "+isDOM2);
   ...
   function loadController()
   {
      ...
      var name = unescape(document.location.href.substring(0,document.location.href.indexOf('#')));
   ...
   function LoadSimple(src, container, code, defer, title, type, language)
   {
      ...
      if(src)
      {
         ...
         if(code) { savedCode = code; code = null; }
      }
   ...
   function publishAPI(THIS)
   {
      ...
      cloakObject(this.Include   = Include);
   ...
   function ShowLog()
   {
      ...
      var logWin = window.open("","__AJILELOG__","width="+width+",height="+height
   ...
   function SimpleSet()
   {
      ...
      if(this.constructor != SimpleSet)         // FBI: Fix Broken Instantiation
         if(!this.constructor || this.constructor.toString() != SimpleSet.toString())
            return new SimpleSet();             // Ensure proper instantiation.
